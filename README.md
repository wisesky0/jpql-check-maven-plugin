...
### 오류 3 (규칙 D-10) — PRD2
FROM 다중 루트 선언 후 JOIN ON 절에서 해당 JOIN이 속하지 않은 다른 FROM 루트를 참조한 경우.

예:
```java
queryFactory
    .from(qRootA, qRootB)                     // FROM 다중 루트
    .innerJoin(qEntityC)
        .on(qEntityC.field.eq(qRootA.field))  // ★ qRootA는 JOIN 소속 트리 밖 → 오류
```
→ Hibernate 6 `org.hibernate.query.SemanticException` 런타임 예외 발생
(`SqmQualifiedJoin predicate referred to SqmRoot [...] other than the join's root [...]`)

Hibernate 6은 명시적 JOIN을 **마지막 FROM 루트의 트리**에 소속시키고, ON 절에서는
같은 트리(JOIN 대상 자신, 소속 루트, 형제 JOIN)만 참조를 허용합니다.
...
- **오류 3 정상 패턴 (감지하지 않음)**:
  - FROM 루트가 1개인 경우
  - ON 절에서 마지막 FROM 루트(JOIN 소속 루트) 또는 형제 JOIN을 참조하는 경우
  - 교차 루트 조건이 WHERE 절에 있는 경우 (WHERE는 범위 제한 없음)
...
[D-10] MyRepository.java:[35,8] JOIN ON 절이 해당 JOIN이 속한 루트 트리 밖의
       FROM 루트(qRootA)를 참조합니다 — Hibernate 6 SemanticException 발생
  JOIN 소속 루트 : qRootB (마지막 FROM 루트)
  ON 절 위반 참조: qRootA
  FROM 루트 목록 : qRootA, qRootB
  권장 조치     : 의도한 루트(qRootB)로 교정하거나, 교차 조건을 WHERE 절로 이동하거나,
...
### 오류 3 우회 — ON 절 교차 루트 참조 해소

```java
// 수정 전 (오류 발생 — qRootA는 JOIN 소속 트리 밖)
.from(qRootA, qRootB)
.innerJoin(qEntityC).on(qEntityC.field.eq(qRootA.field))

// 수정 후 방법 1: 교차 조건을 WHERE 절로 이동
.from(qRootA, qRootB)
.innerJoin(qEntityC).on(qEntityC.field.eq(qRootB.field))
.where(qEntityC.field2.eq(qRootA.field))

// 수정 후 방법 2 (권장): FROM 다중 루트를 명시적 JOIN 체인으로 재구성
.from(qRootB)
.innerJoin(qRootA).on(qRootA.key.eq(qRootB.key))
.innerJoin(qEntityC).on(qEntityC.field.eq(qRootA.field))  // 같은 트리 → 합법

// 수정 후 방법 3: self-join Q 인스턴스(q/q02) 혼동 오타라면 의도한 루트로 교정
```
